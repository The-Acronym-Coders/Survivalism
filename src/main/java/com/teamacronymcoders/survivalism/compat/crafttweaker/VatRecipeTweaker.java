...
public class VatRecipeTweaker {
	...
	private static class AddRecipe implements IAction {
		...
		public String describe() {
			...
			if (outputStack != null && !outputStack.isEmpty()) sb.append("Output Stack: ").append(outputStack.getDisplayName()).append(" ");
			if (itemChance > 0) sb.append("Item Chance: ").append(itemChance).append(" ");
	...
	private static class AddBoots implements IAction {
		...
		@Override
		public void apply() {
			if (boots != Ingredient.EMPTY) CraftTweakerAPI.logError("Cannot register boot multiplier for nothing.");
