...
public class MixingHandler {
    ...
    public void onMixingPre(MixingEvent.Pre event) {
        if (event.isCanceled()) {
            return;
        }
        ...
        if (event.getEntityLiving() instanceof EntityPlayer) {
            ...
            if (!GameStageHelper.hasAllOf(player, stages)) {
                player.sendStatusMessage(new TextComponentTranslation("error.mixing"), true);
    ...
    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public void onMixingPost(MixingEvent.Post event) {
        if (event.isCanceled()) {
            return;
        }
