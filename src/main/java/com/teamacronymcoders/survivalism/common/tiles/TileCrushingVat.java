...
public class TileCrushingVat extends TileEntity implements IUpdatingInventory {
	...
	public void onJump(EntityPlayer jumper) {
		...
		if (jumps >= curRecipe.getJumps() && canInsertResults()) {
			if (HelperMath.tryPercentage(curRecipe.getItemChance())) outputInv.insertItem(0, curRecipe.getOutputStack().copy(), false);
	...
	protected boolean canInsertResults() {
		if (tank.getFluid() == null && (curRecipe.getOutputStack().isEmpty() || outputInv.getStackInSlot(0).isEmpty())) return true;
		Fluid inTank = tank.getFluid().getFluid();
		FluidStack output = curRecipe.getOutput();
		if (!(inTank.getName().equals(output.getFluid().getName()) && tank.getFluidAmount() + output.amount <= tank.getCapacity())) return false;
		ItemStack out = curRecipe.getOutputStack();
		ItemStack cur = outputInv.getStackInSlot(0);
		if (cur.isEmpty()) return true;
		if (!out.isEmpty() && !ItemHandlerHelper.canItemStacksStack(cur, out)) return false;
		if (!out.isEmpty() && cur.getCount() + out.getCount() > cur.getMaxStackSize()) return false;
		return true;
	}
	...
	public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing facing) {
		if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) {
			...
			} else if (facing == EnumFacing.DOWN) { return CapabilityItemHandler.ITEM_HANDLER_CAPABILITY.cast(outputInv); }
		}

		if (capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
			if (facing == EnumFacing.DOWN) { return CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY.cast(tank); }
		}
