...
public class BrewingRecipe {
    ...
    public BrewingRecipe(ResourceLocation id, FluidStack input, List<Ingredient> inputItems, FluidStack output, int ticks) {
        this.id = id;
        this.input = input;
    ...
    public boolean matches(TileBarrelBrewing barrel) {
        ...
        int counter = 0;
        for (Ingredient i : inputItems) {
            for (int idx = 0; idx < 3; idx++) {
                ItemStack s = barrel.getInv().getStackInSlot(idx);
                if (!matchedInputs.contains(counter) && i.apply(s) && !usedSlots.contains(idx)) {
                    usedSlots.add(idx);
