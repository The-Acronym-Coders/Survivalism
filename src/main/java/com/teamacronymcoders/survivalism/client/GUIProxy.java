...
public class GUIProxy implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		if (te instanceof TileBarrel) return ((TileBarrel) te).getContainer(player.inventory);
		if (te instanceof TileCrushingVat) return new ContainerVat(player.inventory, (TileCrushingVat) te);
		return null;
	}
